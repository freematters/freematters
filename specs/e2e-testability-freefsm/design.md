...
FreeFSM E2E is a general-purpose, agent-driven end-to-end testing framework that ships with freefsm. It enables developers to write natural-language test plans as structured markdown, then have an AI agent execute them against real infrastructure. The agent observes effects, judges pass/fail, and produces detailed logs for debugging. Two CLI sub-commands — `freefsm e2e gen` and `freefsm e2e verify` — cover the full lifecycle from test plan generation to execution and reporting.
...
| ID | Requirement |
|----|-------------|
| FR1 | `freefsm e2e gen` generates structured markdown test plans from a workflow YAML or user prompt |
| FR2 | `freefsm e2e verify` executes a test plan using a Claude agent against real infrastructure |
...
| FR7 | Backed by a `verifier.fsm.yaml` workflow that drives the verification process |
| FR8 | General-purpose — not tightly coupled to freefsm; can test any CLI/workflow tool |
...
| NF3 | Must integrate with existing freefsm CLI patterns (global flags, JSON output, error codes) |
...
- Uses `freefsm run` internally with a `gen.fsm.yaml` workflow (or a simple single-shot prompt)

### 4.2 `freefsm e2e verify` Command

**Responsibility**: Execute a test plan using a Claude agent and produce a test report.

```
freefsm e2e verify <test-plan.md> --test-dir <path> [--model <model>] [-j]
```
...
2. Starts a `verifier.fsm.yaml` workflow run
...
### 4.5 `verifier.fsm.yaml` Workflow
...
### 4.6 `gen.fsm.yaml` Workflow (for `e2e gen`)
...
```json
{
  ...
  "step": 1,
  "content": "Ran `freefsm start workflow.yaml --run-id test-123`",
...
**AC1**: Given a workflow YAML, when `freefsm e2e gen <workflow.yaml>` is run, then a structured markdown test plan is produced covering all reachable state paths.

**AC2**: Given a valid test plan, when `freefsm e2e verify <plan.md> --test-dir ./out` is run, then the agent executes each step and `./out/` contains `transcript.jsonl`, `api.jsonl`, and `test-report.md`.

**AC3**: Given a test plan where step 2 should fail, when `freefsm e2e verify` is run, then `test-report.md` shows step 2 as FAIL with evidence and the overall verdict is FAIL.
...
**AC5**: Given `freefsm e2e verify --test-dir ./out -j`, when the run completes, then JSON output includes `{ "success": true/false, "data": { "verdict": "PASS|FAIL", "steps_passed": N, "steps_failed": M } }`.
...
| E2E (dogfood) | Run `freefsm e2e verify` against a simple 2-state FSM test plan | Self-referential: freefsm tests itself |
...
| Choice | Rationale |
|--------|-----------|
| Claude Agent SDK | Already used by `freefsm run`; consistent architecture |
...
| FSM-backed verification | Dogfoods freefsm's own workflow engine; structured agent execution |
...
| Separate test runner package | Adds complexity; bundling with freefsm enables dogfooding and keeps the tool self-contained |
